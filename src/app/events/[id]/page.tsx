...
export default async function EventDetailsPage({
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      ...
      <div className="mb-8">
        <SocialShare
          title={`Check out ${event.title} on Eventra!`}
          description={event.description || "Amazing event on Eventra"}
          url={
      ...
      {isOrganizer && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
          <p className="text-blue-800 dark:text-blue-200 flex items-center gap-2 mb-3">
            <span className="text-xl">👤</span>
            <span>
              <strong>You are the organizer of this event.</strong> You have
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700 mb-8">
        ...
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white text-xl font-bold">
            {typedEvent.organizer?.full_name?.[0] ||
              typedEvent.organizer?.email?.[0] ||
              "O"}
          </div>
          <div>
        ...
        {!isOrganizer && typedEvent.max_tickets_per_user && (
          <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4 mb-6">
            <p className="text-orange-800 dark:text-orange-200 text-sm flex items-center gap-2">
              <span>⚠️</span>
              <span>
                <strong>Purchase Limit:</strong> Maximum{" "}
                {typedEvent.max_tickets_per_user} ticket(s) per person for this
                event.
                {userTicketCount > 0 &&
                  ` You have already purchased ${userTicketCount} ticket(s).`}
              </span>
            </p>
          </div>
        )}
